...
CREATE UNIQUE INDEX `inventory_stock_product_size_idx` ON `inventory_stock` (`product_id`, `size`);
...
CREATE INDEX `inventory_adjustments_product_size_idx` ON `inventory_adjustments` (`product_id`, `size`);
...
CREATE INDEX `in_person_sales_product_size_idx` ON `in_person_sales` (`product_id`, `size`);
...
CREATE INDEX `orders_fulfilled_paid_at_idx` ON `orders` (`fulfilled`, `paid_at`);
...
CREATE INDEX `order_items_product_size_idx` ON `order_items` (`product_id`, `size`);

INSERT INTO `inventory_stock` (`product_id`, `size`, `base_quantity`, `created_at`, `updated_at`) VALUES
